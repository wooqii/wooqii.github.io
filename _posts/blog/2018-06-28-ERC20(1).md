...
`Burn`이라는 이벤트를 선언합니다. 이 이벤트는 `Burner`라는 색인된(`indexed`) 주소와 `value`라는 `uint256`타입 인자를 받습니다.


`burn`함수 선언 : `_value`라는 `uint256`타입의 값을 가지고 `public`으로 선언합니다.
...
`_who`(소각하려는 토큰보유자)의 잔고는 `_who`에서 `SafeMath`의 `sub`함수(- 역할)를 이용해 `_value`만큼 감소시킵니다.

`totalSupply_` 에서도 `_value`만큼 감소시킵니다.

그 후 `Burn` 이벤트를 호출해서 누구의 잔고를 얼마만큼 감소시켰는지 로그를 나타내고,
...
이 계약은 `cap`이라는 변수를 `uint256`타입으로 `public` 하게 선언을 합니다.
...
`mint` 함수 선언 : `_to`라는 주소값과 `_amount`라는 `uint256`타입의 수량값을 인자로 `bool` 값을 반환하는 public함수입니다.**이 함수를 통해서 우리는 원하는 사람에게 토큰을 발행해줄 수 있습니다!**
